...
const BotStats: BotCommand = {
  ...
  executeSlash: async (
    ...
    try {
      ...
      const activeGuilds = await GuildModel.count({
        where: { isActive: true },
      });
      const inactiveGuilds = await GuildModel.count({
        where: { isActive: false },
      });
      const bannedGuilds = await GuildModel.count({
        where: { status: GuildStatus.BANNED },
      });
      ...
      const embed = createAdvancedEmbed({
        ...
        .addFields([
          ...
          {
            name: "**Inaktif Sunucu**",
            value: `${inactiveGuilds}`,
            inline: true,
          },
          {
            name: "**Banlanmış Sunucu**",
            value: `${bannedGuilds}`,
            inline: true,
          },
        ])
      ...
      const errorEmbed = QuickEmbeds.error(
        "İstatistikleri alırken bir hata oluştu.",
        interaction.user
      );
