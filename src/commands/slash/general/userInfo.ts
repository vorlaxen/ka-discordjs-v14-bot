        ...
        try {
            ...
            const embed = createAdvancedEmbed({
              ...
              .addFields([
                  ...
                  { name: "Sunucuya Katılma", value: member ? `<t:${Math.floor(member.joinedTimestamp!/1000)}:R>` : "Sunucuya katılmamış", inline: true },
                  { name: "Hesap Oluşturulma", value: `<t:${Math.floor(targetUser.createdTimestamp/1000)}:R>`, inline: true },
                  { name: "Roller", value: member ? `${member.roles.cache.size - 1}` : "0", inline: true },
                  { name: "Avatar Linki", value: `[Tıkla](${avatarUrl})`, inline: true }
              ]);

            await replySafe(interaction, embed);
        } catch (err) {
            const errorEmbed = QuickEmbeds.error("Kullanıcı bilgilerini alırken bir hata oluştu.", interaction.user);
